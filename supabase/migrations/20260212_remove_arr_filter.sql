-- Remove ARR > 0 filter from Visit Planner
-- Date: 2026-02-12
-- Purpose: Show all accounts regardless of ARR value (including $0)

-- Drop existing function
DROP FUNCTION IF EXISTS find_nearby_accounts CASCADE;

-- Recreate function without ARR > 0 requirement
...
BEGIN
  ...
  WHERE
    ...
    -- Removed: AND a.arr IS NOT NULL
    -- Removed: AND a.arr > 0
    -- Keep min_arr filter for user preferences
    AND (p_min_arr = 0 OR (a.arr IS NOT NULL AND a.arr >= p_min_arr))
  ORDER BY distance_miles ASC;
END;
$$;

COMMENT ON FUNCTION find_nearby_accounts IS 'Find accounts within radius of a location. Shows all accounts including those with $0 or NULL ARR. Users can still filter by min_arr if desired.';
