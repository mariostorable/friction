...
async function previewMatching() {
  ...
  console.log(`Cases in database: ${cases?.length || 0}\n`);
    ...
    for (const [key, value] of Object.entries(customFields)) {
      ...
      if (caseMatches) {
        ...
        caseMatches.forEach(caseNum => {
          const accountId = caseToAccount.get(caseNum);
          if (accountId) {
  ...
  console.log(`✓ Estimated account-jira links: ~${withCaseIds * 1.2} (avg 1.2 accounts per ticket)\n`);
  ...
  console.log(`\n⚠️  ${accountsWithoutTickets.length} accounts won't have Jira tickets (no Case ID matches)`);
  if (accountsWithoutTickets.length > 0) {
    console.log('\nTop 10 accounts without tickets:');
    accountsWithoutTickets.slice(0, 10).forEach(a => console.log(`  - ${a.name}`));
  }
}
