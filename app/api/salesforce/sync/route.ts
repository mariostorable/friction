    ...
    const fetchSalesforceAccounts = async (accessToken: string) => {
      ...
      if (!fullResponse.ok) {
        ...
        if (errorText.includes('INVALID_FIELD') || errorText.includes('No such column')) {
          ...
          const simpleQuery = `SELECT Id,Name,AnnualRevenue,Industry,Type,Owner.Name,CreatedDate,ShippingStreet,ShippingCity,ShippingState,ShippingPostalCode,ShippingCountry,BillingStreet,BillingCity,BillingState,BillingPostalCode,BillingCountry,Parent_Street__c,Parent_City__c,Parent_State__c,Parent_Zip__c,(SELECT Id FROM Assets) FROM Account WHERE ParentId=null AND ((Industry LIKE '%Storage%' OR Industry LIKE '%Marine%' OR Industry LIKE '%RV%') OR (Name LIKE '% - CORP%' OR Name LIKE '%-CORP%' OR Name LIKE '%CORP.%')) AND (AnnualRevenue>0 OR Name LIKE '% - CORP%' OR Name LIKE '%-CORP%' OR Name LIKE '%CORP.%') ORDER BY AnnualRevenue DESC NULLS LAST LIMIT 1000`;
